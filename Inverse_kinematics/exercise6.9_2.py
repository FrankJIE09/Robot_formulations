...
def inverse_kinematics(theta_list, axis_list, l_list, Tsd, max_iter=100, tol_w=1e-3, tol_v=1e-4):
    hat_v_Tsd = logm(Tsd)
    v_Tsd = hat_to_SE3(hat_v_Tsd.real)

    for _ in range(max_iter):
        T_prev = np.eye(4)
        for theta, axis, l in zip(theta_list, axis_list, l_list):
...
if __name__ == '__main__':
    ...
    l_list = [1, 1]
    inverse_kinematics(theta_list, axis_list, l_list, Tsd, )
